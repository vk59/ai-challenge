#!/usr/bin/env python3
"""День 15: контролируемые переходы в нативном окне macOS.

    ../../.venv/bin/python app.py

На полосе этапов вместо стрелки стоит замок там, где переход заперт условием.
Кнопка «Утвердить» — справа, и нажать её может только человек.
...
WINDOW_TITLE = "Контролируемые переходы"
_alive: list = []
...
def build() -> NSWindow:
    if AGENT.task:
        ...
        subtitle = (f"{AGENT.model} · этап: {AGENT.task.label}"
                    + (f" · заперто: {', '.join(заперто)}" if заперто else ""))
    else:
        subtitle = f"{AGENT.model} · задача не заведена"
    port = start_server()
