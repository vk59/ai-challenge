#!/bin/bash
#
# Собирает «День 12.app» — персонализированный ассистент.
#
#   ./build_app.sh
#   open "dist/День 12.app"
#
# Устройство как в днях 6-11: рантайм, .env и память живут в
# ~/Library/Application Support — macOS не пускает .app в ~/Documents,
# а пересборка не должна стирать ни переписку, ни профили.
...
# Весь shared целиком: agent.py тянет llm, memory и tokens. Каждый забытый
# файл — падение на старте, проверено на днях 7 и 8.
...
<plist version="1.0">
<dict>
  <key>CFBundleName</key>                <string>День 12</string>
  <key>CFBundleDisplayName</key>         <string>День 12 — персонализация</string>
  <key>CFBundleIdentifier</key>          <string>local.aiadvent.day12</string>
...
mkdir -p "$MEMORY"
echo "  память и профили: $MEMORY (пересборка их не трогает)"
...
echo
echo "Проверка дня: задайте вопрос, нажмите «Сравнить профили» —"
echo "один и тот же вопрос уйдёт через все профили, ответы встанут рядом."
