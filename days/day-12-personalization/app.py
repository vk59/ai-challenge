#!/usr/bin/env python3
"""День 12: персонализированный ассистент в нативном окне macOS.

    ../../.venv/bin/python app.py

То же окно, что в дне 11 (чаты слева, память справа), плюс панель профиля
и кнопка «Сравнить профили»: один вопрос уходит через все профили сразу,
и ответы встают рядом. По одному ответу персонализацию не проверить —
непонятно, профиль подействовал или модель так ответила.
...
WINDOW_TITLE = "Персонализированный ассистент"
_alive: list = []
...
class Actions(NSObject):
    ...
    def compareProfiles_(self, sender) -> None:
        self.webview.evaluateJavaScript_completionHandler_("startCompare()", None)
...
def build_menu(actions: Actions) -> None:
    ...
    chat_menu.addItem_(NSMenuItem.separatorItem())
    item(chat_menu, "Сравнить профили", "compareProfiles:", "d", actions)
...
def build_window(port: int, subtitle: str) -> NSWindow:
    ...
    window = NSWindow.alloc().initWithContentRect_styleMask_backing_defer_(
        NSMakeRect(0, 0, 1180, 760), style, NSBackingStoreBuffered, False)
...
def build() -> NSWindow:
    профили = ", ".join(p.name for p in AGENT.profiles()) or "нет"
    port = start_server()
    ...
    return build_window(port, f"{AGENT.model} · профили: {профили}")
