#!/usr/bin/env python3
"""День 6: агент в нативном окне macOS.
...
def build() -> NSWindow:
    ...
    # Подзаголовок окна берём у самого агента — он и есть источник правды
    # о том, какая модель используется и что он помнит.
    model = AGENT.model
    port = start_server()
