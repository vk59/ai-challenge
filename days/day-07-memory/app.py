#!/usr/bin/env python3
"""День 7: агент с памятью в нативном окне macOS.
...
Проверка задания здесь самая наглядная: написать что-нибудь, закрыть окно
на ⌘Q — то есть убить процесс целиком, вместе со всей оперативной памятью, —
и открыть снова. Разговор будет на месте.
...
WINDOW_TITLE = "Память между запусками"
...
class Actions(NSObject):
    ...
    def eraseConversation_(self, sender) -> None:
        self.webview.evaluateJavaScript_completionHandler_("eraseConversation()", None)
...
def build_menu(actions: Actions) -> None:
    ...
    dialog_menu.addItem_(NSMenuItem.separatorItem())
    # Стирание отделено от создания нового диалога: одно необратимо, другое нет.
    item(dialog_menu, "Стереть этот диалог", "eraseConversation:", "", actions)
...
def build_window(port: int, subtitle: str) -> NSWindow:
    ...
    window.setSubtitle_(subtitle)        # вторая строка в титлбаре, macOS 11+
...
def build() -> NSWindow:
    ...
    # Подзаголовок окна берём у самого агента: модель и то, сколько пар
    # он только что поднял с диска. Видно сразу при запуске.
    subtitle = f"{AGENT.model} · архив: {AGENT.archived} пар"
    port = start_server()
